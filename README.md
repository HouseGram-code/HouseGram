...
[![Что нового](https://img.shields.io/badge/📋_Что_нового-Docker_fixes_✅-brightgreen?style=for-the-badge)](docker/compose/DEPLOY_FIX.md)

<details>
<summary><b>🔧 Последние исправления (клик для раскрытия)</b></summary>

### Исправления Docker Compose

| Сервис | Было | Стало |
|--------|------|-------|
| gateway-server | segfault (root/ICU) | Dockerfile исправлен ✅ |
| file-server | несуществующий образ → 139 | локальная сборка (Node.js) ✅ |
| session-server | pull access denied | локальная сборка (.NET) ✅ |
| sms-sender | pull access denied (риск) | отключён ✅ |
| version: "3.9" | WARN | удалён ✅ |

**Подробности:** см. [DEPLOY_FIX.md](docker/compose/DEPLOY_FIX.md)
